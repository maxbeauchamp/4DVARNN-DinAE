...
def learning_OSSE(dict_global_Params,genFilename,\
    ...
    NbGradIter     = [2,2,7,9,10,12,15]
    #NbGradIter     = [0,0,0,0,0,0,0]
    if flagTrWMissingData==2:
        lrUpdate       = [1e-4,1e-5,1e-6,1e-7]
    else:
        lrUpdate       = [1e-3,1e-3,1e-5,1e-6]
        print("lr_update=",lrUpdate)
    #IterUpdate     = [0,3,10,15,20,25,30,35,40]
    IterUpdate = [0, 12, 13, 15, 20, 25, 30, 35, 40]
    val_split      = 0.1
    iterInit       = 0
    comptUpdate    = 0  
            ...
            for phase in ['train', 'val']:
                ...
                for inputs_init, masks, targets in dataloaders[phase]:
                    ...
                    with torch.set_grad_enabled(True): 
                        ...
                        loss_All    = torch.mean((outputs[:,5,:,:] - targets[:,5,:,:])**2 )
                        if N_cov>0:
                        ...
                        spatial_gradients_avg = einops.reduce(sobel(outputs[:,5:6,:,:]), 'b t lat lon -> 1', 'mean')
