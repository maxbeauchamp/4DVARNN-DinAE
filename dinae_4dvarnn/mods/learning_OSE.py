            ...
            for phase in ['train', 'val']:
                ...
                for inputs,mask_inputs,targets,mask_targets,NATL60 in dataloaders[phase]:
                    ...
                    with torch.set_grad_enabled(True): 
                        ...
                        # full DAW
                        idT1 = np.arange(0,inputs.shape[1],N_cov+1)
                        ...
                        spatial_gradients_avg = einops.reduce(sobel(outputs[:,idT2,:,:]), 'b t lat lon -> 1', 'mean')
                        loss_Obs1    = torch.sum( (outputs[:,idT2,:,:] - inputs[:,idT1,:,:])**2 * mask_inputs[:,idT1,:,:])
